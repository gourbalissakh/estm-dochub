...
export async function GET(request: NextRequest) {
  ...
  const messages = await prisma.message.findMany({
    ...
    include: { author: { select: { firstName: true, lastName: true, avatarUrl: true } }, filiere: true },
...
export async function POST(request: NextRequest) {
  ...
  const message = await prisma.message.create({
    data: { content: parsed.data.content, filiereId: parsed.data.filiereId, authorId: session.user.id },
    include: { author: { select: { firstName: true, lastName: true } }, filiere: true },
  });
