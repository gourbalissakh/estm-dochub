...
export default function AdminUploadPage() {
  ...
  async function submit() {
    ...
    for (let index = 0; index < totalChunks; index += 1) {
      ...
      Object.entries({ ...form, filiereId: form.filiereId || filieres[0]?.id, fileName: file.name, fileType: file.type || "application/pdf", fileSize: String(file.size), uploadId, chunkIndex: String(index), totalChunks: String(totalChunks) }).forEach(([k, v]) => body.set(k, v));
    ...
    <Card>
      ...
      <CardContent className="grid gap-4">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <select className="h-10 rounded-md border px-3 dark:bg-[#14101F]" onChange={(e) => setForm({ ...form, filiereId: e.target.value })}>{filieres.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select>
        ...
        <div className="h-2 overflow-hidden rounded-full bg-violet-100"><div className="h-full bg-cyan-500" style={{ width: `${progress}%` }} /></div>
